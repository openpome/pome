...
Repository knowledge is stored inside the selected workspace:

```txt
<workspace>/.pome/knowledge/repository.json
```

This is metadata only: package/build/test maps, source/test/config/generated/sensitive path maps, module boundaries, and ownership signals. It does not store source contents, secrets, prompts, diffs, or AI responses.
...
`pome` is the daily home screen with the active story, tool readiness, and one next action. `pome onboard` is the friendly setup screen: it checks Jira, GitHub, and AI readiness, then shows assigned work when Jira is connected so the next command is obvious.
...
OpenPome supports native GitHub browser/device login when your organization configures a GitHub OAuth app for CLI use:
...
The token is stored through the OS credential store. For alpha compatibility, OpenPome still falls back to GitHub CLI auth when no OpenPome GitHub OAuth client is configured:
