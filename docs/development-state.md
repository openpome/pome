...
Current version: `0.46.0-alpha.0`.
...
- First-run CLI guidance is improved for `pome`, `pome init`, `pome doctor`, and `pome help` in the current `0.46.0-alpha.0` development version.
...
- OpenAI quota/billing failures are classified separately from normal provider rate limits, and the CLI guides developers to add credits, switch to Claude CLI/Claude API, or use `pome ai context` instead of blindly retrying.
...
2. Publish `0.46.0-alpha.0` after the AI provider quota guidance PR lands.
...
4. Create GitHub release `v0.46.0-alpha.0` with alpha boundaries and install instructions.
...
are complete through 0.35.0-alpha.0; main is now 0.46.0-alpha.0. SQLite
...
knowledge are in place. Continue by publishing 0.46.0-alpha.0 with fresh npm
