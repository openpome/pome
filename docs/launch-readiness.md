...
- Version consistency: all packages and gateway health use `0.46.0-alpha.0`.
...
- `pome onboard` shows a product-style welcome screen with simple Jira/GitHub/AI readiness, one next action, and an assigned-work preview when Jira is ready.
- `pome auth jira token` is the primary Jira setup path for developers; it validates Jira API-token credentials against Jira before storing them through the OS credential store and avoids shell-profile setup for normal use.
- Successful Jira token setup shows the verified Jira account and accessible board information, so developers can trust that the connection is real.
- `pome work` now explains when assigned work is filtered by a saved board/scope, and `pome work all` lists all Jira issues assigned to the developer without that scope filter for one run.
...
- OpenAI quota/billing failures are classified separately from normal rate limits, and the CLI guides developers to add credits, switch provider, or use `pome ai context` instead of blindly retrying.
...
- Publish `0.46.0-alpha.0` after the AI provider quota guidance PR lands.
- Sync npm `latest` to `0.46.0-alpha.0` if npm refuses to delete stale alpha `latest` tags, so default installs do not receive old CLI builds.
- Create a GitHub release for `v0.46.0-alpha.0`.
