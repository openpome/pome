...
- `pome pr create` performs the explicit GitHub write step through OpenPome's GitHub API token when browser login is connected, or through GitHub CLI fallback after the developer asks for it.
...
OPENPOME_GITHUB_OAUTH_SMOKE=I_UNDERSTAND_THIS_USES_REAL_GITHUB_OAUTH pnpm smoke:github-oauth
